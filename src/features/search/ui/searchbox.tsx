...
export default function Searchbox(props: SearchboxProps): ReactNode {
		...
		if (isDropdownOpen && dropdownRef.current != null) {
			...
			if (selectedItem != null) {
				...
				if (selectedItem.offsetTop >= scrollThreshold) {
					dropdownRef.current.scrollTop = selectedItem.offsetTop - dropdownHeight + selectedItem.clientHeight
				}
			}
		}
	}, [isDropdownOpen])

	const filteredEntries = props.entries.filter(entry => entry.title.toLowerCase().includes(searchQuery.toLowerCase()))
	...
	return (
		<div className='relative' ref={containerRef}>
			...
			{isDropdownOpen && (
				<div
					className='absolute p-0 top-full flex flex-col max-h-[35rem] overflow-y-scroll mt-2 left-0 w-full bg-neutral-800 rounded-sm border border-stone-700'
					ref={dropdownRef}
				>
